...
import 'fashion_models_provider.dart';
...
final fashionModelsListProvider =
    NotifierProvider<FashionModelsNotifier, List<FashionModel>>(
        FashionModelsNotifier.new);

class FashionModelsNotifier extends Notifier<List<FashionModel>> {
  @override
  List<FashionModel> build() {
    return _fashionModels;
  }
    ...
    state = [
      for (FashionModel model in state)
        if (model == fashionModel)
          fashionModel..modifyLikeCount(like)
        else
          model
    ];
  }
